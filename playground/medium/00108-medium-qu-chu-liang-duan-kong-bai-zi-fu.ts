...
type Space = ' ' | '\n' | '\t'
type TrimMy<S extends string> = S extends `${Space}${infer Left}` ? Trim<Left> :
  S extends `${infer Left}${Space}` ? Trim<Left> : S;

type Trim<S extends string> = S extends `${Space}${infer Left}` | `${infer Left}${Space}` ? Trim<Left> : S;
